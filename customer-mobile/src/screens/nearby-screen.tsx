...
function formatDistance(meters: number): string {
  ...
  if (isCorrectMeter >= 1000) return `${(isCorrectMeter / 1000).toFixed(2)} km`;
  return `${Math.round(isCorrectMeter)} m`;
}
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          ...
          <View
            style={{
              ...
              paddingHorizontal: 6,
              borderRadius: 6,
              backgroundColor: outlet.isOpen ? "#dcfce7" : c.muted,
            }}
          >
            <Text
              style={{
                fontSize: 9,
                fontWeight: "600",
                color: outlet.isOpen ? "#16a34a" : c.mutedForeground,
              }}
            >
        ...
        >
          ...
          {outlet.duration != null && (
            ...
            >
              ...
              <Text style={{ fontSize: 10, color: c.mutedForeground }}>
                {formatDuration(outlet.duration)}
              </Text>
            </View>
          )}
        </View>
...
function MapInitialFetch({
  ...
  const { isLoaded, mapRef } = useMap();

  useEffect(() => {
    if (!isLoaded || !mapRef.current) return;
    mapRef.current.getBounds().then((bounds) => {
      const [west, south, east, north] = bounds;
      onRegionChange({ west, south, east, north });
    });
  }, [isLoaded, mapRef, onRegionChange]);
...
function MapViewMode({
  ...
  const handleRegionDidChange = useCallback(
    (event: { nativeEvent: { bounds: [number, number, number, number] } }) => {
  ...
  return (
    ...
    >
      <MapInitialFetch onRegionChange={onRegionChange} />
...
export default function NearbyScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== "granted") {
        setLocationError(
          "Izin lokasi diperlukan untuk menampilkan outlet terdekat",
        );
        return;
      }
  ...
  const allOutlets = useMemo(
    () => nearbyData?.pages.flatMap((p) => p.data) || [],
    [nearbyData],
  );

  const fetchMapOutlets = useCallback(
    async (bounds: {
      west: number;
      south: number;
      east: number;
  ...
  const handleRegionChange = useCallback(
    (bounds: { west: number; south: number; east: number; north: number }) => {
      ...
      fetchTimerRef.current = setTimeout(() => fetchMapOutlets(bounds), 500);
    },
    [fetchMapOutlets],
  );
        ...
        <>
                ...
                {[1, 2, 5, 10, 20].map((d) => (
                  <Pressable
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      backgroundColor:
                        selectedDistance === d ? c.primary : `${c.primary}10`,
                    }}
                  >
                    <Text
                      style={{
          ...
          {isLoading ? (
            ...
            >
              ...
              >
                {search
                  ? `Tidak ada outlet untuk "${search}"`
                  : `Tidak ada outlet dalam radius ${selectedDistance} km`}
              </Text>
            </View>
          ) : (
            <FlatList
      ...
      >
        ...
        >
          <List
            size={14}
            color={
              viewMode === "list" ? c.primaryForeground : c.mutedForeground
            }
          />
          <Text
            style={{
        ...
        <Pressable
          ...
          style={{
            ...
            paddingHorizontal: 16,
            borderRadius: 20,
            backgroundColor: viewMode === "map" ? c.primary : "transparent",
          }}
        >
          <MapIcon
            size={14}
            color={viewMode === "map" ? c.primaryForeground : c.mutedForeground}
          />
          <Text
            style={{
              fontSize: 12,
              fontWeight: "600",
              color:
                viewMode === "map" ? c.primaryForeground : c.mutedForeground,
            }}
          >
            Peta
          </Text>
