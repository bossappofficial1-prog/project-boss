        ...
        <div class="absolute inset-y-0 right-0 max-w-full flex">
          <div class="relative w-screen max-w-md">
            <div class="h-full flex flex-col bg-white dark:bg-gray-900 shadow-2xl">
              <!-- Header -->
              <div class="px-6 py-6 bg-gradient-to-r from-red-600 to-red-700 dark:from-red-700 dark:to-red-800">
                ...
                <div v-if="!cartStore.isEmpty" class="mt-4 text-white/90 text-sm flex items-center gap-2">
                  <Icon name="lucide:package" class="h-4 w-4" />
                  <span>{{ cartStore.totalItems }} item{{ cartStore.totalItems > 1 ? 's' : '' }} di keranjang</span>
                </div>
              </div>
              ...
              <div class="flex-1 overflow-y-auto py-6">
                ...
                <div v-if="cartStore.isEmpty" class="text-center py-16 px-6">
                  ...
                  <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Keranjang Kosong</h3>
                  <p class="text-gray-500 dark:text-gray-400 mb-6">Mulai berbelanja untuk menambahkan item ke keranjang
                    Anda</p>
                  <button @click="cartStore.toggleCart"
                      ...
                      <div class="flex-1">
                        <div class="flex justify-between items-start mb-2">
                          ...
                          <button @click="cartStore.removeItem(item.product.id)"
                            ...
                            <Icon name="lucide:trash-2" class="h-3 w-3 text-red-600 dark:text-red-400" />
                          </button>
                        </div>
                        ...
                        <div class="flex justify-between items-end">
                          ...
                          <div class="flex items-center gap-3">
                            <button @click="cartStore.updateQuantity(item.product.id, item.quantity - 1)"
                              ...
                              <Icon name="lucide:minus" class="h-4 w-4 text-gray-600 dark:text-gray-300" />
                            </button>

                            <span class="w-8 text-center font-semibold text-gray-900 dark:text-white text-sm">
                              {{ item.quantity }}
                            </span>
                ...
                <div class="px-6 py-6">
                  ...
                  <div class="space-y-3 mb-6">
                    <div class="flex justify-between text-sm">
                      <span class="text-gray-600 dark:text-gray-400">Subtotal ({{ cartStore.totalItems }} item{{
                        cartStore.totalItems > 1 ? 's' : '' }})</span>
                      <span class="font-semibold text-gray-900 dark:text-white">{{ formatCurrency(cartStore.totalPrice)
                        }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
