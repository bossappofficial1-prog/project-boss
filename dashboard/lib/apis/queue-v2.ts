...
export const queueV2Api = {
  async getBoard(outletId: string, q?: string, dateStr?: string): Promise<QueueV2BoardResponse> {
    return apiCall<QueueV2BoardResponse>(
      `/queue/v2/${outletId}/board?q=${q || ""}&date=${dateStr || ""}`,
    );
  },

  async transitionStatus(orderId: string, status: QueueOrderStatus, reason?: string): Promise<any> {
