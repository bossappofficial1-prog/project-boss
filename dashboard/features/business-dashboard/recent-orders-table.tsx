...
export function RecentOrdersTable({ orders }: RecentOrdersTableProps) {
        ...
        return (
          <div className="flex justify-center">
            <Badge
              variant="outline"
              className={cn(
                "rounded-md text-[10px] font-bold px-2 py-0",
                status.className,
              )}
            >
              {status.label}
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex justify-center">
          <Badge
            variant="secondary"
            className="rounded-md text-[10px] font-semibold bg-muted/50"
          >
            {ORDER_STATUS_LABEL[row.original.orderStatus] ||
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-medium text-foreground whitespace-nowrap">
            {formatISOStringDate(row.original.createdAt)}
          </span>
  ...
  return (
    <DataTable
      ...
      description="Aktivitas transaksi terakhir dari seluruh jaringan outlet."
      columns={columns}
      data={orders}
