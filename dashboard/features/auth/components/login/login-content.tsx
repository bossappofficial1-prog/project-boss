...
export default function LoginContent() {
  ...
  const handleVerify2fa = async () => {
    ...
    try {
      const res = await apiClient.post("/auth/2fa/authenticate", {
        tempToken,
        token: twoFactorCode,
        trustDevice,
      });
      ...
      const redirectTo = redirectUrl?.startsWith("/") && !redirectUrl.startsWith("/auth/")
        ? redirectUrl
        : userRole === "OWNER"
          ? "/owner"
          : userRole === "ADMIN"
  ...
  return (
    <AuthSplitLayout>
      <div className="w-full max-w-md space-y-6">
        ...
        <form onSubmit={twoFactorStep ? handleVerify2fa : handleSubmit} className="space-y-4">
          {twoFactorStep ? (
            <>
              <div className="text-center py-2">
                <Smartphone className="w-10 h-10 text-primary mx-auto mb-2" />
                <p className="text-sm font-medium">Verifikasi Dua Faktor</p>
