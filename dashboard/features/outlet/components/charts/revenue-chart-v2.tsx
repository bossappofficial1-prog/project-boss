...
import { useOutletRevenueTrend } from "@/hooks/use-outlet-revenue-trend";
...
export default function RevenueChart({ outletId }: RevenueChartProps) {
  ...
  const { data: trend, isPending } = useOutletRevenueTrend(outletId, {
    timeframe,
    ...(timeframe === "custom"
      ? {
  ...
  if (isPending) {
    return <Skeleton className="h-112.5 w-full rounded-md" />;
  }

  return (
    <Card className="rounded-md gap-0 py-0 border-border/60 shadow-md overflow-hidden bg-linear-to-b from-background to-muted/5">
      <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-border/40 bg-muted/20 p-4">
        <div className="space-y-1">
          ...
          <CardDescription className="text-xs">
            Pertumbuhan omzet harian outlet Anda.
          </CardDescription>
        </div>
          ...
          {timeframe === "custom" && (
            <DatePickerWithRange
              {...(customRange !== null && {
                date: customRange,
                onDateChange: (date) =>
                  setCustomRange({ from: date.from!, to: date.to! }),
              })}
            />
          )}
        </div>
      ...
      <CardContent className="p-0">
        {isPending ? (
          <Skeleton className="h-112.5 w-full rounded-md" />
        ) : (
          <>
            {/* Hero Stats Section */}
            <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x border-b border-border/40 bg-muted/10">
              <div className="p-4 space-y-1 group">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                  <DollarSign className="h-3 w-3 text-emerald-500" /> Total
                  Pendapatan
                </p>
