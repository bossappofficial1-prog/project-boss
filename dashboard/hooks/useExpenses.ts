...
export function useExpenses(outletId?: string | null): UseExpensesResult {
	...
	const fetchData = useCallback(async () => {
		...
		try {
			setLoading(true);
			setError(null);
			const list = await expenseApi.listByOutlet(outletId, { startDate: startISO, endDate: endISO });
			setExpenses(list || []);
		} catch (err: any) {
	...
	const create = useCallback(async (payload: { description: string; amount: number; date: string; outletId: string }) => {
		await expenseApi.create(payload);
		await fetchData();
	}, [fetchData]);

	const update = useCallback(async (id: string, payload: Partial<Expense>) => {
