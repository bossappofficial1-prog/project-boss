...
const formatScheduleInfo = (entry: QueueEntry) => {
  const raw =
    ...
    entry.queueMeta?.scheduledStart ??
...
const primaryActionMap: Partial<
  Record<
    OrderStatus,
    { nextStatus: OrderStatus; label: string; title: string; confirmLabel: string }
  >
> = {
  AWAITING_PAYMENT: {
...
export function useQueueStatusActions({ onSuccess }: UseQueueStatusActionsOptions = {}) {
  ...
  const requestStatusChange = useCallback(
    (entry: QueueEntry, nextStatus: OrderStatus, overrides?: QueueActionOverrides) => {
      const currentStatus = resolveStatus(entry);
  ...
  const executeStatusUpdate = useCallback(
    async (confirmReason?: string) => {
      ...
      try {
        await orderApi.updateServiceStatus(entry.id, nextStatus, confirmReason);
