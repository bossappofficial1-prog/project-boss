...
export default function ReportOutlerContent() {
  ...
  // Sync outletFilter when selectedOutlet changes from outside (e.g. sidebar),
  // but only if we are not already on 'all' or specifically selected?
  // Actually, usually reports page follows selected outlet.
  // But if we want "All", we might want to stay on "All".
  // Let's set initial state.
  ...
  const { data: reportOutletData, isLoading } = useReportOutlet(
    outletFilter,
    filterType,
  ...
  const totals = useMemo<Totals>(() => {
    return (reportOutletData || []).reduce(
      (acc, curr) => ({
  ...
  return (
    <>
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Pantau Penjualan, Stok, Biaya & Komisi.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-[#1e293b] rounded-md border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden relative">
        ...
        <ReportOutleTable data={reportOutletData || []} totals={totals} />
      </div>
    </>
  );
}
