  ...
  const fields: FormFieldConfig<any>[] = [
    ...
    },
    {
      ...
      placeholder: isEditMode
        ...
        : "Minimal 6 karakter",
      description: "Kredensial ini digunakan kasir untuk masuk ke aplikasi POS.",
      condition: (values) => !values.role || values.role === "CASHIER",
    },
    ...
    },
    {
      ...
      type: "password",
      placeholder: isEditMode ? "Biarkan kosong jika tidak ingin mengubah" : "Masukkan 6 digit angka PIN",
      description: "Kredensial PIN angka 6 digit yang digunakan manager untuk login.",
      condition: (values) => values.role === "MANAGER",
    },
    {
      name: "privileges",
      ...
      renderCustom: ({ field, form }) => {
            ...
            items: [
              {
                ...
                description: "Mengelola detail info outlet, kapasitas meja, dan reservasi meja aktif.",
                icon: Store
              },
              {
                value: "ORDER_MANAGEMENT",
                label: "Manajemen Pesanan",
                description: "Melihat dan mengelola daftar pesanan operasional kasir secara real-time.",
                icon: ShoppingBag
              }
            ]
          },
            ...
            items: [
              {
                ...
                description: "Menambah, mengubah, dan menghapus produk, kategori, serta layanan katalog.",
                icon: Package
              },
              {
                value: "SERVICE_MANAGEMENT",
                label: "Manajemen Layanan",
                description: "Mengakses kalender booking, jadwal operasional, dan slot reservasi layanan.",
                icon: Calendar
              }
            ]
          },
            ...
            items: [
              {
                ...
                label: "Stok & Supplier",
                description: "Mengelola penyesuaian stok, logistik pengadaan, dan kontak supplier/distributor.",
                icon: Truck
              },
              {
                value: "CUSTOMER_MANAGEMENT",
                label: "Pelanggan & Loyalty",
                description: "Mengelola database pelanggan, program loyalty point, dan tingkat membership.",
                icon: UsersIcon
              }
            ]
          },
            ...
            items: [
              {
                ...
                description: "Melihat laporan keuangan, mencatat pengeluaran operasional, dan menutup shift kasir.",
                icon: Wallet
              },
              {
                value: "TRANSACTION_VIEW",
                label: "Riwayat Transaksi",
                description: "Melihat keseluruhan daftar struk, pembayaran, dan status transaksi di outlet.",
                icon: FileText
              }
            ]
          },
            ...
            items: [
              {
                ...
                description: "Otoritas tinggi untuk menyetujui request hapus transaksi kasir atau hapus langsung.",
                icon: Trash2,
                isSensitive: true
              }
            ]
            ...
            items: [
              {
                ...
                description: "Akses grafik performa, jam ramai pelanggan, margin profit, dan laporan laba rugi.",
                icon: TrendingUp
              },
              {
                value: "TOOLS_CALCULATOR",
                label: "Alat Operasional",
                description: "Menggunakan kalkulator BEP, Kalkulator HPP produk, serta target breakdown.",
                icon: Calculator
              }
            ]
          }
        ...
        const allPrivilegeValues = privilegeGroups.flatMap((g) => g.items.map((i) => i.value));
        const isAllSelected = allPrivilegeValues.every((val) => currentPrivileges.includes(val));

        const handleSelectAll = () => {
          if (isAllSelected) {
            form.setValue("privileges", [], { shouldValidate: true });
          } else {
            form.setValue("privileges", allPrivilegeValues, { shouldValidate: true });
          }
        };
        ...
        const handleCheckboxChange = (val: string, checked: boolean) => {
          ...
          form.setValue("privileges", updatedPrivileges, { shouldValidate: true });
        };
        ...
        let authorityDescription = "Hanya dapat membuka modul-modul dasar yang dicentang.";
        let authorityColorClass = "text-amber-500 bg-amber-500/10 border-amber-500/20";
        let progressColorClass = "bg-amber-500";
        
        if (percent === 100) {
          authorityLevel = "Super Manager (Setara Owner)";
          authorityDescription = "Wewenang penuh operasional, keuangan, dan data produk outlet.";
          ...
          authorityDescription = "Wewenang luas mencakup mayoritas fungsi supervisi.";
          authorityColorClass = "text-blue-500 bg-blue-500/10 border-blue-500/20";
          progressColorClass = "bg-blue-500";
        } else if (percent === 0) {
          authorityLevel = "Tanpa Akses Modul";
          authorityDescription = "Akun terdaftar tanpa wewenang operasional apapun.";
        ...
        const quickTemplates = [
          ...
          },
          {
            ...
            icon: ShoppingBag,
            values: ["OUTLET_MANAGEMENT", "ORDER_MANAGEMENT", "SERVICE_MANAGEMENT", "CUSTOMER_MANAGEMENT"],
          },
          {
            name: "Fokus Keuangan & Stok",
            description: "Stok, Katalog & Laporan",
            icon: Wallet,
            values: ["PRODUCT_MANAGEMENT", "STOCK_MANAGEMENT", "FINANCE_REPORTS", "TRANSACTION_VIEW", "ANALYTICS", "TOOLS_CALCULATOR"],
          },
        ];
        ...
        return (
          <div className="space-y-6 mt-3 animate-fadeInUp">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-border">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Pilih hak akses operasional yang ingin Anda delegasikan ke Manager ini.
                </p>
              </div>
              <button
                type="button"
              ...
              >
                <Check className={cn("h-3.5 w-3.5 transition-transform duration-200", isAllSelected ? "scale-100" : "scale-75")} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Stats card */}
              <div className={cn("p-4 rounded-xl border flex flex-col justify-between gap-2.5 transition-all duration-300", authorityColorClass)}>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase tracking-wider">Level Otoritas</span>
                  <Sparkles className="h-4 w-4 shrink-0" />
                </div>
                <div className="space-y-0.5">
                  <div className="text-sm font-bold tracking-tight">{authorityLevel}</div>
                  <div className="text-[10px] opacity-80 leading-normal">{authorityDescription}</div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl border border-border bg-card/60 flex flex-col justify-between gap-3 md:col-span-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-foreground">Kekuatan Akses Modul</span>
                  <span className="font-bold text-muted-foreground">{selectedCount} dari {totalCount} Modul Terpilih ({percent}%)</span>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-muted-foreground leading-normal flex items-start gap-1.5">
                    ...
                    <span>Manager hanya bisa melihat menu & melakukan aksi pada dashboard sesuai dengan hak akses yang Anda centang di bawah ini.</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-border bg-card/40 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {quickTemplates.map((tpl) => {
                  const isTemplateMatch = tpl.values.length === currentPrivileges.length && 
                  ...
                  return (
                    <button
                      key={tpl.name}
                      type="button"
                      onClick={() => form.setValue("privileges", tpl.values, { shouldValidate: true })}
                      className={cn(
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {privilegeGroups.map((group) => {
                ...
                const isGroupAllSelected = groupValues.every(val => currentPrivileges.includes(val));
                const selectedInGroupCount = groupValues.filter(val => currentPrivileges.includes(val)).length;
                ...
                const handleToggleGroup = (e: React.MouseEvent) => {
                  ...
                  if (isGroupAllSelected) {
                    ...
                    updated = updated.filter(val => !groupValues.includes(val));
                  } else {
                  ...
                  form.setValue("privileges", updated, { shouldValidate: true });
                };
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2.5">
                      {group.items.map((item) => {
                        const isChecked = currentPrivileges.includes(item.value);
                        ...
                        return (
                          <label
                            ...
                            className={cn(
                              ...
                              item.isSensitive && !isChecked && "hover:border-destructive/30"
                            )}
                          >
                            {/* Checkbox Icon */}
                            <div className={cn(
                            ...
                            <div className="flex-1 space-y-1">
                              <div className="flex items-center gap-1.5">
                                <ItemIcon className={cn("h-4 w-4 shrink-0", isChecked ? "" : "text-muted-foreground/80")} />
                                <span className="text-xs sm:text-sm font-bold select-none leading-none">{item.label}</span>
                                {item.isSensitive && (
            ...
            {currentPrivileges.includes("TRANSACTION_DELETE") && (
              <div className="flex gap-3 p-4 rounded-xl border border-destructive/20 bg-destructive/5 text-xs text-destructive/90 leading-relaxed shadow-xs animate-fadeIn">
                ...
                <div className="space-y-0.5">
                  <span className="font-bold text-destructive">PERINGATAN HAK AKSES SENSITIF</span>
                  <p>Anda memberikan wewenang <strong>Hapus Transaksi</strong> kepada Manager ini. Hak ini bersifat destruktif dan dapat menghapus atau merubah riwayat transaksi finansial outlet, yang dapat mempengaruhi laporan laba rugi serta audit keuangan.</p>
                </div>
              </div>
            )}
          </div>
