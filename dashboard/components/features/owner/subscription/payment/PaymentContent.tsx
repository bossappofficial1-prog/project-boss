...
const BANK_ACCOUNTS = [
    { bank: 'BCA', accountNumber: '1234567890', accountHolder: 'PT BOSS APP INDONESIA' },
    { bank: 'Mandiri', accountNumber: '0987654321', accountHolder: 'PT BOSS APP INDONESIA' },
    { bank: 'BNI', accountNumber: '1122334455', accountHolder: 'PT BOSS APP INDONESIA' },
]
...
export default function SubscriptionPaymentContent({ invoiceId }: { invoiceId: string }) {
    ...
    const { mutateAsync: uploadProof, isPending: isUploading } = useUploadInvoiceProof()
    ...
    const isUploadable = invoice.status === 'PENDING' || invoice.status === 'REJECTED_MANUAL'
    ...
    return (
        <div className="min-h-screen bg-background px-4 py-6">
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <Button variant="ghost" size="sm" className="gap-1.5" onClick={() => router.push('/owner/subscription')}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                    <div className="space-y-1">
                        <h1 className="text-2xl font-semibold tracking-tight">Konfirmasi Pembayaran</h1>
                        <p className="font-mono text-sm text-muted-foreground">{invoice.invoiceNumber}</p>
                    </div>
                    <Badge className={`border text-sm font-semibold w-fit ${PAYMENT_STATUS_STYLES[invoice.status]}`}>
                ...
                {uploadSuccess && (
                    <div className="flex gap-3 rounded-lg border-l-4 border-emerald-500 bg-muted/50 p-4">
                        ...
                        <div>
                            <p className="text-sm font-medium">Bukti pembayaran berhasil diunggah</p>
                ...
                {isVerified && (
                    <div className="flex gap-3 rounded-lg border-l-4 border-emerald-500 bg-muted/50 p-4">
                        ...
                        <div>
                            <p className="text-sm font-medium">Pembayaran telah diverifikasi</p>
                            <p className="text-sm text-muted-foreground mt-0.5">Langganan Anda sudah aktif.</p>
                        </div>
                    </div>
                )}
                ...
                {invoice.status === 'REJECTED_MANUAL' && invoice.rejectionReason && (
                    <div className="flex gap-3 rounded-lg border-l-4 border-destructive bg-destructive/5 p-4">
                        ...
                        <div>
                            <p className="text-sm font-medium text-destructive">Pembayaran ditolak</p>
                            <p className="text-sm text-muted-foreground mt-0.5">{invoice.rejectionReason}</p>
                            <p className="text-sm text-muted-foreground">Silakan unggah ulang bukti yang valid.</p>
                        </div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <Card>
                            <CardHeader className="pb-3">
                                <CardTitle className="text-base font-medium">Pilih Bank Tujuan</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="grid grid-cols-3 gap-3">
                                    {BANK_ACCOUNTS.map((item, idx) => (
                                        ...
                                        >
                                            <div className={`mx-auto mb-2 h-10 w-10 rounded-full flex items-center justify-center text-xs font-bold
                        ...
                        <Card>
                            <CardHeader className="pb-3">
                                <CardTitle className="text-base font-medium">Detail Transfer</CardTitle>
                            </CardHeader>
                    ...
                    <div className="lg:col-span-1">
                        <Card className="sticky top-4">
                            <CardHeader className="pb-3">
                                <CardTitle className="text-base font-medium">Ringkasan Invoice</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                <div className="rounded-lg bg-muted/50 p-4 text-center">
                                    <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Total Tagihan</p>
                                ...
                                <div className="space-y-3 text-sm">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <Badge className={`border text-xs font-semibold ${PAYMENT_STATUS_STYLES[invoice.status]}`}>
                                            {PAYMENT_STATUS_LABELS[invoice.status]}
                                        </Badge>
                                    </div>
                                    <Separator />
