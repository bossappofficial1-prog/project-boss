        ...
        <section className="grid gap-4 lg:grid-cols-12">
                    ...
                    <div className="space-y-1.5">
                        <div className="flex items-center gap-3">
                            ...
                            {overview.business?.subscriptionStatus && (
                                <Badge className={cn('px-2.5 py-0.5 rounded-md border text-[10px] font-black uppercase tracking-widest shadow-none', SUBSCRIPTION_STATUS_STYLES[overview.business.subscriptionStatus])}>
                        ...
                        <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                            ...
                            ID Paket: {plan?.code || 'N/A'} • {plan?.durationDays ?? '-'} Hari Aktif
                        </p>
                    </div>
                    <div className="text-right">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Biaya Paket</p>
                ...
                <CardContent className="p-6 space-y-6">
                    <div className="grid gap-4 sm:grid-cols-3">
                        ...
                        <div className={cn(
                            ...
                            <div>
                                <p className="text-[10px] font-bold uppercase tracking-widest opacity-70">Sisa Masa Aktif</p>
                                <p className="text-sm font-black italic">
                                    {daysLeft === null ? '-' : daysLeft === 0 ? 'HARI INI' : `${daysLeft} HARI`}
                                </p>
                            </div>
                    ...
                    <div className="flex flex-col gap-3 rounded-md border border-border/60 bg-muted/20 p-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-bold text-foreground uppercase tracking-wide">{overview.business?.name}</span>
                        </div>
                        <div className="text-[10px] font-medium text-muted-foreground italic">
                            Status sinkronisasi terakhir: {formatDateTime(overview.business?.subscriptionStartDate ?? '')}
                        </div>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card className="lg:col-span-4 gap-0 pb-0 rounded-md overflow-hidden border-border/80 bg-background shadow-sm">
                <CardHeader className="border-b border-border/40 bg-muted/30">
                    <CardTitle className="text-sm font-black uppercase tracking-widest text-foreground">Fitur Paket Aktif</CardTitle>
                </CardHeader>
                <CardContent className="p-6">
                    {planFeatures ? (
                        <ul className="space-y-4">
                            {[
                                { icon: Store, label: "Maksimal Outlet", value: formatLimitLabel(planFeatures.maxOutlets) },
                                { icon: Package2, label: "Produk & Layanan", value: formatLimitLabel(planFeatures.maxProducts) },
                                { icon: Users, label: "Tim Staf", value: formatLimitLabel(planFeatures.maxStaff) },
                                { icon: FileText, label: "Ekspor Laporan", value: planFeatures.canExportReport ? 'Tersedia' : 'Tidak Tersedia' },
                                { icon: ReceiptText, label: "Support Level", value: planFeatures.supportLevel === 'PRIORITY' ? 'Priority Direct' : planFeatures.supportLevel === 'WHATSAPP' ? 'WA Bisnis' : 'Email' }
                            ].map((feat, idx) => (
                                <li key={idx} className="flex items-center gap-3 group">
                                    <div className="p-1.5 rounded-md bg-muted text-muted-foreground group-hover:bg-foreground group-hover:text-background transition-colors border border-border/80">
                                    ...
                                    <div className="flex-1 border-b border-border/40 pb-1 flex items-center justify-between">
                                        ...
                                        <span className="text-xs font-black text-foreground">{feat.value}</span>
                                    </div>
                                </li>
                            ))}
                        </ul>
                        ...
                        <div className="py-10 text-center space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground font-medium italic">Fitur paket tidak terbaca.</p>
                        </div>
                    )}

                    <Button
                        variant="default"
                        className="w-full mt-6 h-11 gap-2 font-black uppercase tracking-widest text-[10px] shadow-sm"
                        onClick={handleRenew}
