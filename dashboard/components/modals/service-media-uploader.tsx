...
export default function ServiceMediaUploader({
  ...
  const handleAddEmbed = useCallback(async () => {
    ...
    if (ytMatch) {
      ...
      finalUrl = `https://www.instagram.com/${contentType}/${mediaId}/embed`;
    }
  ...
  return (
    <div className="space-y-3">
          ...
          {value.map((item, idx) => {
            ...
            return (
              <div
                ...
                className={`relative aspect-[9/16] rounded-md overflow-hidden border-2 group cursor-grab active:cursor-grabbing transition-all ${
                  dragOverIndex === idx
        ...
        <div className="flex gap-2 items-end">
          <div className="flex-1 space-y-1">
            <Label className="text-xs text-muted-foreground">
              URL YouTube atau Instagram (Disarankan YouTube Shorts)
            </Label>
            <Input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && (e.preventDefault(), handleAddEmbed())
              }
            />
          </div>
      ...
      <p className="text-xs text-muted-foreground">
        Foto: maks 5MB (jpg, png, webp) &bull; Drag untuk mengurutkan
      </p>
    </div>
  );
}
