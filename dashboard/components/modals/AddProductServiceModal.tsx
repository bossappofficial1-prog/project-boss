...
export default function AddOrEditProductServiceModal({ open, onOpenChange, initialData, outletId, onSuccess, action = 'edit' }: Props) {
    ...
    const defaultValues = useMemo<ProductFormValues>(() => {
        if (isEdit && initialData) {
            ...
            if (initialData.type === 'SERVICE') {
                return {
                    ...
                    service: {
                        durationMinutes:
                            initialData.service?.durationMinutes ?? 30,
                        sellingPrice:
                            initialData.service?.sellingPrice ?? 0,
                        providerName: initialData.service?.providerName ?? '',
                        commissionType: initialData.service?.commissionType ?? 'PERCENTAGE',
        ...
        return {
            ...
            service: { commissionType: 'PERCENTAGE' } as any,
            file: undefined,
        }
    ...
    const handleSubmit = async (values: ProductFormValues | FormData) => {
        ...
        if (formType === 'GOODS') {
            ...
            const service: ServiceSchemaType = {
                ...
                providerEmail: otherValues.get('service[providerEmail]') as string,
                providerPhone: otherValues.get('service[providerPhone]') as string
            }
    ...
    const fields: FormFieldConfig<ProductFormValues>[] = [
        ...
        },
        {
            ...
            colSpan: 3,
            condition: (values) => values.type === 'SERVICE',
            typeResolver: (values) => values.service?.commissionType == 'FIXED' ? 'currency' : 'presentage',
        },
        {
