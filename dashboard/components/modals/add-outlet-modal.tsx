...
const outletSchema = z.object({
  ...
  phone: z
    ...
    .refine(
      ...
      {
        message:
          "Masukkan nomor telepon yang valid (contoh: 081234567890 atau +6281234567890)",
      },
    ),
...
export default function AddOutletModal({
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && outlet) {
      form.reset({
        ...
        type: outlet.type || OutletType.CUSTOM,
  ...
  // Update with detailed data (operating hours) when query resolves
  useEffect(() => {
  ...
  }, [outletDetail, open, mode]);

  const handleOperatingHoursChange = useCallback(
    (data: Record<number, OperatingHoursFormData>) =>
  ...
  const { mutate, isPending: isSubmitting } = useMutation({
    mutationFn: async (values: any) => {
      ...
      const base = {
        ...
        latitude:
          latValue && latValue !== "undefined" ? Number(latValue) : undefined,
        longitude:
          lngValue && lngValue !== "undefined" ? Number(lngValue) : undefined,
      ...
      const changedHours = Object.values(operatingHoursData).filter(
        (d) => d.isOpen !== undefined,
      );
      try {
        ...
        ...(mode === "edit" && outletDetail?.id
          ? [
              queryClient.invalidateQueries({
                queryKey: ["outlet-detail", outletDetail.id],
              }),
            ]
          : []),
      ]);
    ...
    () => [
        ...
        options: [
          {
            ...
            value: OutletType.FNB,
          },
          {
            label: "Retail (Barang/Stok)",
            value: OutletType.RETAIL,
          },
          {
            label: "Jasa (Layanan/Booking)",
            value: OutletType.SERVICE,
          },
          {
            label: "Event (Tiket/Check-in)",
            value: OutletType.EVENT,
          },
          {
            label: "Custom (Semua Fitur)",
            value: OutletType.CUSTOM,
          },
        ].filter((opt) => {
      ...
      },
      {
        ...
        renderCustom: ({
          ...
          field: ControllerRenderProps<OutletFormData, Path<OutletFormData>>;
        }) => (
          <MapPicker
            ...
            renderMarkerContent={() => (
              <div className="flex items-center justify-center h-6 w-6 rounded-full bg-primary/90 ring-2 ring-white shadow">
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto p-0">
        <SheetHeader className="border-b border-border/60 sticky top-0 bg-background z-10">
