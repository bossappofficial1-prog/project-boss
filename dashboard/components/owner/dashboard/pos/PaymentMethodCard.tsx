      ...
      <CardContent className="space-y-4">
          ...
          <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/60">
            ...
            {!!cashReceived && cashChange !== 0 && (
              <p
                ...
                {cashChange < 0
                  ? "Nominal masih kurang"
                  : `Kembalian: Rp ${cashChange.toLocaleString("id-ID")}`}
              </p>
            )}
          </div>
        )}
        ...
        {paymentMethod === "qris" && (
          <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-400">
            ...
            <p className="text-xs">
              Pastikan pelanggan menunjukkan bukti pembayaran. Status transaksi tetap menunggu
              verifikasi.
            </p>
          </div>
        )}
        ...
        {paymentMethod === "online" && (
          <div className="space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-slate-600 dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-400">
            <p className="text-xs">
              Pilih kanal pembayaran digital. Sistem akan membuat transaksi Midtrans Core API sesuai
              pilihan ini.
            </p>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Setelah pesanan dibuat, kasir akan melihat kode bayar atau instruksi pembayaran sesuai
              kanal yang dipilih.
            </p>
          </div>
        )}
