		...
		<div className="space-y-3">
			<div key={item.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
				<div className="flex items-center justify-between">
					...
					<div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{fmtCurrency(item.amount)}</div>
				</div>
				<div className="mt-3 flex items-center gap-3 justify-end">
					<button onClick={() => onEdit(item)} className="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 text-sm">Edit</button>
					<button onClick={() => onDelete(item)} className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 text-sm">Hapus</button>
				</div>
			</div>
		</div>
	);
