      ...
      render={({ field: formField }) => (
        <FormItem
          className={cn(
            ...
            field.oneLine && "flex flex-row items-center justify-between space-y-0 gap-4",
            field.className,
          )}>
          <FormLabel className={cn(field.oneLine && "min-w-fit mb-0 mt-0")} htmlFor={field.name}>
            {field.label}
          </FormLabel>
          <FormControl className={cn("col-span-1", field.oneLine && "flex-1")}>
            <FieldInputSwitch
              allValues={values}
