    ...
    const columns = useMemo<ColumnDef<OutletTable>[]>(() => [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                const config = {
                    AVAILABLE: { label: 'Tersedia', class: 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' },
                    OCCUPIED: { label: 'Terisi', class: 'bg-red-500/10 text-red-600 border-red-500/20' },
                    RESERVED: { label: 'Dipesan', class: 'bg-amber-500/10 text-amber-600 border-amber-500/20' }
                }[status] || { label: status, class: 'bg-muted text-muted-foreground' }

                return (
                    <Badge variant="outline" className={cn("text-[10px] font-black uppercase tracking-tighter", config.class)}>
                        {config.label}
                    </Badge>
                )
            }
