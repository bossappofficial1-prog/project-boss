...
import {
    X, MapPin, Phone, Clock, Store, Pencil, QrCode, Globe, Zap, ArrowRightLeft, Save
} from 'lucide-react'
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="space-y-4">
                    <Card className="rounded-md gap-py-0 border-border/40 bg-muted/5">
                        ...
                        <CardContent><Skeleton className="aspect-square w-full rounded-xl opacity-40" /></CardContent>
                    </Card>
                </div>
            </div>
        </div>
...
export default function ManageOutletContent() {
    const { selectedOutlet, setSelectedOutlet, isLoading: outletLoading } = useOutletContext()
    ...
    const handleSave = async (values: FormData) => {
        ...
        if (outletImage instanceof File) {
            nextImageUrl = (await uploadApi.uploadImage(outletImage, { scope: 'outlet' })).url
        }
        if (qrisImage instanceof File) {
            nextQrImageUrl = (await uploadApi.uploadImage(qrisImage, { scope: 'outlet' })).url
        }
        ...
        try {
            const updateData: Partial<Outlet> = {
                ...
                ...(nextQrImageUrl !== undefined ? { manualQrImageUrl: nextQrImageUrl } : {}),
            }

            const updatedOutlet = await outletManagementApi.update(selectedOutlet.id, updateData)
    ...
    const { mutate: submit, isPending: isSaving } = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['outlet', selectedOutlet?.id] })
        }
    })
    ...
    const formFields = useMemo<FormFieldConfig<OutletFormValues>[]>(() => [
        ...
        },
        {
            ...
            type: 'file',
            description: 'Upload QRIS static terbaru agar pelanggan bisa membayar tanpa kendala.',
        ...
        },
        {
            ...
            description: 'Nama ini akan tampil di dashboard, struk, dan halaman pelanggan.',
            colSpan: 6,
        },
        {
            name: 'isOpen',
            ...
            description: 'Atur status buka/tutup agar pelanggan melihat ketersediaan outlet secara real-time.',
            colSpan: 6,
            switchOptions: {
                left: { label: 'Tutup', value: false, activeClass: 'text-rose-500 font-bold' },
                right: { label: 'Buka', value: true, activeClass: 'text-emerald-500 font-bold' }
            }
        },
        {
            name: 'phone',
        ...
        },
        {
            ...
            description: 'Gunakan kalimat singkat yang menjelaskan karakter dan keunikan outlet.',
            className: 'min-h-24',
            colSpan: 12,
        },
        {
            ...
            description: 'Alamat rinci membantu kurir, tamu, dan pelanggan menemukan lokasi dengan cepat.',
            className: 'min-h-28',
            colSpan: 12,
        },
        {
            ...
            type: 'custom',
            colSpan: 12,
            description: 'Geser pin di peta agar koordinat outlet akurat untuk pelacakan dan navigasi.',
            renderCustom: ({ values, form }) => (
                <div className="space-y-2">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                        Koordinat: {(values.latitude || 0).toFixed(6)}, {(values.longitude || 0).toFixed(6)}
                    </p>
                </div>
            )
        },
            ...
            <SectionHeader
                ...
                badge={
                    <div className="flex items-center gap-2">
                        <Badge variant={selectedOutlet.isOpen ? 'success' : 'destructive'} className="px-3 py-0.5 rounded-md border border-current/20 text-[10px] font-black uppercase tracking-widest bg-current/5 shadow-none">
                            {selectedOutlet.isOpen ? 'Buka' : 'Tutup'}
                        </Badge>
                        <Badge variant="outline" className="px-3 py-0.5 rounded-md border-border/80 text-[10px] font-black uppercase tracking-widest bg-muted/20 text-muted-foreground shadow-none">
                    ...
                    <Card className="rounded-md border-border/80 gap-0 py-0 bg-background shadow-sm overflow-hidden">
                        <CardHeader className="border-b gap-0 pt-4 border-border/40 bg-muted/10 flex flex-row items-center justify-between">
                            <div className="space-y-1">
                                <CardTitle className="text-sm font-black uppercase tracking-widest">Edit Profil Outlet</CardTitle>
                                <CardDescription className="text-[10px] font-bold uppercase tracking-wider">Perbarui informasi dasar dan visual outlet Anda.</CardDescription>
                            </div>
                            <Button variant="ghost" size="icon" onClick={() => setIsEditing(false)} className="h-8 w-8 rounded-full">
                                <X className="h-4 w-4" />
                            </Button>
                        </CardHeader>
                            ...
                            <Card className="rounded-md overflow-hidden gap-0 py-0 border-border/80 bg-background shadow-sm">
                                <div className="relative h-48 overflow-hidden bg-muted">
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-4 bg-linear-to-t from-black/80 to-transparent text-white">
                                        <h2 className="text-xl font-black tracking-tight">{selectedOutlet.name}</h2>
                                        <p className="text-[10px] font-black uppercase tracking-widest opacity-70">Profil Outlet</p>
                                    </div>
                                </div>
                                <CardContent className="p-4">
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Biodata & Deskripsi</p>
                                        <p className="text-sm font-medium leading-relaxed text-foreground/80 italic">
                                            {selectedOutlet.description || 'Tidak ada deskripsi untuk outlet ini.'}
                                        </p>
                                    </div>
                                </CardContent>
                            </Card>
                                    ...
                                    <div className="grid gap-2">
                                        <div className="flex items-center gap-3 rounded-md border border-border/40 bg-muted/20 p-3">
                                            ...
                                            <div className="space-y-0.5">
                                                ...
                                                <p className="text-sm font-black text-foreground">{selectedOutlet.phone || '-'}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-start gap-3 rounded-md border border-border/40 bg-muted/20 p-3">
                                            ...
                                            <div className="space-y-0.5">
                                                ...
                                                <p className="text-sm font-bold text-foreground/90 leading-relaxed">{selectedOutlet.address || '-'}</p>
                                            </div>
                                        </div>
                                        {selectedOutlet.latitude && (
                                            <div className="flex items-center gap-3 rounded-md border border-border/40 bg-muted/20 p-3">
                                                ...
                                                <div className="space-y-0.5">
                                                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Koordinat</p>
                        ...
                        <div className="space-y-4">
                                    ...
                                    {selectedOutlet.manualQrImageUrl ? (
                                        <div className="space-y-4">
                                            <div className="relative overflow-hidden rounded-md border border-border/80 bg-card p-4 shadow-sm">
                                                <img src={selectedOutlet.manualQrImageUrl} alt="QRIS" className="mx-auto aspect-square w-full object-contain" />
                                                <div className="absolute top-3 right-3">
                                                    <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[8px] font-black uppercase tracking-tighter shadow-none">Terverifikasi</Badge>
                                                </div>
                                            </div>
                                            <p className="text-[10px] text-center font-bold text-muted-foreground uppercase tracking-widest">Scan untuk melakukan pembayaran digital.</p>
                                        </div>
                                    ) : (
                                        <div className="aspect-square flex flex-col items-center justify-center rounded-md border border-border/60 bg-muted/20 text-muted-foreground/40">
                                            ...
                                            <p className="text-[10px] font-black uppercase tracking-widest text-center px-4">QRIS belum dikonfigurasi</p>
                                        </div>
                                    )}
                                </CardContent>
                            </Card>
                            ...
                            <Card className="rounded-md gap-0 py-0 border-border/80 bg-background shadow-sm overflow-hidden">
                                ...
                                <CardContent className="p-4">
                                    <div className="space-y-4">
                                        {[
                                            { label: "Operasional", value: selectedOutlet.isOpen ? 'Buka' : 'Tutup', variant: selectedOutlet.isOpen ? 'success' : 'destructive' },
                                            { label: "Payment", value: selectedOutlet.manualQrImageUrl ? 'Tersedia' : 'Belum Ada', variant: selectedOutlet.manualQrImageUrl ? 'success' : 'outline' },
                                            { label: "Geolokasi", value: selectedOutlet.latitude ? 'Tersedia' : 'Belum Set', variant: selectedOutlet.latitude ? 'success' : 'outline' }
                                        ].map((item, idx) => (
                                            <div key={idx} className="flex items-center justify-between">
                                                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-tight">{item.label}</span>
                                                <Badge variant={item.variant as any} className="px-2 py-0 rounded-md border text-[9px] font-black uppercase tracking-tighter shadow-none">{item.value}</Badge>
                                            </div>
                                        ))}
                                    </div>
                                </CardContent>
