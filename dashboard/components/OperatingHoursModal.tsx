  ...
  const handleSave = async () => {
    ...
    for (const hour of Object.values(operatingHoursData)) {
      if (hour.isOpen && hour.openTime >= hour.closeTime) {
        toast.error(
          `Jam buka harus lebih kecil dari jam tutup untuk ${getDayLabel(hour.dayOfWeek)}`,
        );
        return;
      }
    }
    ...
    try {
      ...
      await upsertMutation.mutateAsync({
        ...
        hours: Object.values(operatingHoursData).map((hour) => ({
          ...
          isOpen: hour.isOpen,
          isRestEnabled: hour.isRestEnabled || false,
          restStartTime: hour.restStartTime
            ? new Date(`1970-01-01T${hour.restStartTime}:00`)
            : undefined,
          restEndTime: hour.restEndTime ? new Date(`1970-01-01T${hour.restEndTime}:00`) : undefined,
        })),
      });
