...
export default function OutletsDashboard() {
  ...
  const {
    ...
    productTypeData,
    topPaymentMethod,
  } = useOutletAnalytics(selectedOutlet?.id);
  ...
  return (
    <div className="space-y-3 pb-6">
      <Card className="rounded-md gap-0 py-0 overflow-hidden border-border/60 shadow-md bg-linear-to-br from-background to-muted/20">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b border-border/40 bg-muted/30 p-6">
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2 text-muted-foreground">
              <Badge
                variant="outline"
                className="bg-background/50 font-semibold px-2 py-0.5 rounded-sm border-primary/20 text-primary"
              >
          ...
          {dashboardData?.payments && (
            <div className="hidden sm:flex flex-col items-end">
              ...
              <Badge
                variant="outline"
                className="rounded-md px-3 py-1.5 text-sm font-bold bg-emerald-500/10 text-emerald-600 border-emerald-500/20"
              >
        ...
        <CardContent className="grid grid-cols-1 gap-6 p-6 sm:grid-cols-3">
          ...
          <div className="space-y-2 group">
            ...
            <p className="text-2xl font-black tracking-tighter text-foreground">
              {dashboardData?.orders?.todayOrders?.toLocaleString("id-ID") ??
                "0"}
              <span className="ml-1.5 text-xs font-medium text-muted-foreground uppercase tracking-normal italic">
                Pesanan
              </span>
            </p>
          </div>
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          {lowStockProducts.length ? (
            <Card className="rounded-md gap-0 pt-0 border-border/60 shadow-sm overflow-hidden bg-linear-to-b from-background to-amber-500/5 h-full">
              <CardHeader className="flex flex-row items-center justify-between border-b border-border/40 bg-amber-500/5 p-4">
                ...
                <Badge
                  variant="outline"
                  className="rounded-md px-2.5 py-1 text-xs font-bold bg-amber-100 text-amber-700 border-amber-200 uppercase"
                >
              ...
              <CardContent className="p-4 space-y-2">
                {lowStockProducts.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between p-3 rounded-lg border border-border/50 bg-background/50 hover:border-amber-200 hover:bg-amber-50/50 transition-all group"
                  >
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase font-medium">
                        Minimal Restok: {item.reorderLevel}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-black text-amber-600 italic">
          ...
          {dashboardData?.payments ? (
            <Card className="rounded-md gap-0 pt-0 border-border/60 shadow-sm overflow-hidden bg-linear-to-b from-background to-primary/5 h-full">
              <CardHeader className="border-b border-border/40 bg-muted/20 p-4">
                <CardTitle className="text-lg font-bold">
                  Insight & Manual Payment
                </CardTitle>
              </CardHeader>
                ...
                <div className="p-4 rounded-xl border border-border/50 bg-background/50 space-y-2">
                  ...
                  {topPaymentMethod && (
                    <p className="text-[10px] font-medium text-muted-foreground italic">
                      {topPaymentMethod.count} Transaksi •{" "}
                      {formatCurrency(topPaymentMethod.amount)}
                    </p>
                  )}
                </div>
                <div className="p-4 rounded-xl border border-border/50 bg-background/50 space-y-2">
                  ...
                  <p className="text-[10px] font-medium text-muted-foreground italic">
                    {dashboardData.payments.successCount} Berhasil •{" "}
                    {dashboardData.payments.failedCount} Gagal
                  </p>
                </div>
              </CardContent>
                ...
                <CardContent className="px-4 pb-4">
                  <div className="p-4 rounded-xl border border-amber-200/50 bg-amber-500/5 space-y-4">
                    <div className="flex items-center justify-between border-b border-amber-200/30 pb-2">
                      ...
                      <Badge
                        variant="outline"
                        className="bg-amber-500 text-white border-none text-[10px] font-black h-5"
                      >
                        PENDING: {dashboardData.payments.manualPayments.pending}
                      </Badge>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm font-bold">
                          {dashboardData.payments.manualPayments.verified}
                        </span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-amber-700/60 uppercase">
                        ...
                        <span className="text-sm font-bold">
                          {dashboardData.payments.manualPayments.rejected}
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <Card className="rounded-md gap-0 py-0 border-border/60 shadow-md overflow-hidden bg-linear-to-b from-background to-red-500/5">
          <CardHeader className="flex flex-row items-center justify-between border-b border-border/40 bg-muted/20 p-4">
            ...
            <Badge
              variant="outline"
              className="rounded-md bg-red-500/10 border-red-200 text-red-700 px-3 py-1.5 text-xs font-bold uppercase tracking-tight"
            >
              Total Hari Ini:{" "}
              {formatCurrency(dashboardData?.expenses?.todayExpenses || 0)}
            </Badge>
          </CardHeader>
