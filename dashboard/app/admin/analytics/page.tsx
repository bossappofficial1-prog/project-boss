...
export default function AdminAnalytics() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get(
        `/admin/analytics/transactions?${params}`,
      );
      return response.data.data;
    },
  });
      ...
      {insights && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {formatCurrency(insights.summary.mrr)}
              </div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${insights.summary.revenueGrowth >= 0 ? "text-emerald-600" : "text-destructive"}`}
              >
                {insights.summary.revenueGrowth >= 0 ? "+" : ""}
                {insights.summary.revenueGrowth.toFixed(1)}%
              </div>
              <p className="text-xs text-muted-foreground">
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="font-bold text-primary">
                          {formatCurrency((item as any).totalAmount)}
                        </p>
                        <p className="text-xs text-muted-foreground">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="text-center">
                <p className="text-2xl font-bold text-emerald-600">
                  {transactionStats?.summary?.successfulTransactions || 0}
                </p>
                <p className="text-sm text-muted-foreground">Successful</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-destructive">
                  {transactionStats?.summary?.failedTransactions || 0}
                </p>
                <p className="text-sm text-muted-foreground">Failed</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-primary">
                  {transactionStats?.summary?.successRate?.toFixed(1) || 0}%
                </p>
      ...
      <Card>
        ...
        <CardContent>
          {revenueLoading ? (
            ...
            <div className="space-y-4">
              {insights?.topBusinesses
                ?.slice(0, 5)
                .map((business: TopBusiness, index: number) => (
                  <div
                    key={business.id}
              ...
              {(!insights?.topBusinesses ||
                insights.topBusinesses.length === 0) && (
                <p className="text-sm text-muted-foreground text-center py-8">
