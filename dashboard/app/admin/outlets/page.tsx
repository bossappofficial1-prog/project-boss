...
import { useGetAdminOutlets, useUpdateOutletForceClose, useDeleteOutlet } from "@/hooks/api/use-admin-control"
...
export default function OutletManagement() {
    ...
    const handleForceClose = async () => {
        ...
        try {
            ...
            toast.success(`Outlet berhasil di${selectedOutlet.isOpen ? 'tutup' : 'buka'} secara paksa`);
    ...
    const columns = React.useMemo<ColumnDef<any>[]>(() => [
                ...
                return (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs text-muted-foreground">ID: {outlet.id.split('-')[0]}</span>
                        </div>
                    </div>
                );
            }
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="text-sm">
                        ...
                        <div className="text-xs text-muted-foreground">{outlet.business?.owner?.name || 'N/A'}</div>
                    </div>
                );
            }
        },
        {
            ...
            cell: ({ row }) => {
                const outlet = row.original;
                return (
                    <div className="text-sm text-muted-foreground max-w-[200px] truncate flex items-center gap-1.5">
                        <MapPin className="h-3 w-3 shrink-0" />
                        <span className="truncate">{outlet.address || 'Tidak ada alamat'}</span>
                    </div>
                );
            }
        },
            ...
            cell: ({ row }) => {
                ...
                return (
                    <Badge variant="outline" className={
                        outlet.isOpen ? "bg-emerald-500/10 text-emerald-600 border-emerald-200" :
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        ...
                                        <SheetDescription className="flex items-center gap-2 text-xs">
                                            <MapPin className="h-3 w-3" /> {selectedOutlet.address || 'Alamat belum diatur'}
                                        </SheetDescription>
                                    </div>
                                    <div className="flex flex-col items-end gap-2">
                                        <Badge className={
                                            selectedOutlet.isOpen ? "bg-emerald-500 hover:bg-emerald-600" : "bg-destructive"
                                        }>
                                            ...
                                            <Card className="shadow-md border-border/40 bg-gradient-to-br from-card to-muted/20 hover:border-primary/20 transition-all duration-300">
                                                <CardHeader className="p-4 pb-2">
                                                    <CardTitle className="text-xs font-semibold text-muted-foreground flex items-center gap-2">
                                                        <Clock className="h-4 w-4 text-primary/70" /> Jam Operasional
                                                    </CardTitle>
                                                </CardHeader>
                                                <CardContent className="p-4 pt-0">
                                                    <div className="text-sm font-bold text-foreground">Mengikuti Jadwal</div>
                                                </CardContent>
                                            </Card>
                                            <Card className="shadow-md border-border/40 bg-gradient-to-br from-card to-muted/20 hover:border-primary/20 transition-all duration-300">
                                                <CardHeader className="p-4 pb-2">
                                                    <CardTitle className="text-xs font-semibold text-muted-foreground flex items-center gap-2">
                                                        <Phone className="h-4 w-4 text-primary/70" /> Kontak Outlet
                                                    </CardTitle>
                                                </CardHeader>
                                                <CardContent className="p-4 pt-0">
                                                    <div className="text-sm font-bold text-foreground">{selectedOutlet.phone || '-'}</div>
                                                </CardContent>
                                            </Card>
                                        </div>
                                    </TabsContent>
                                    ...
                                    <TabsContent value="operations" className="space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="p-4 rounded-lg border border-border/50 bg-card shadow-sm">
                                                ...
                                                <div className="text-2xl font-bold">{selectedOutlet._count?.staff || 0}</div>
                                            </div>
                                            <div className="p-4 rounded-lg border border-border/50 bg-card shadow-sm">
                                                <div className="flex items-center gap-2 text-muted-foreground mb-2">
                                                ...
                                                <div className="text-2xl font-bold">{selectedOutlet._count?.products || 0}</div>
                                            </div>
                                        </div>
                                    </TabsContent>
                                </Tabs>
