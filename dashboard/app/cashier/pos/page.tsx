...
import { AlertTriangle, Calendar, Clock, Copy, Minus, Plus } from "lucide-react";
...
} from "@/components/owner/dashboard/pos/ServiceScheduleDialog";
...
export default function CashierPOSPage() {
  ...
  const filteredProducts = React.useMemo(() => {
    const list =
      activeFilter === "ALL" ? products : products.filter((item) => item.type === activeFilter);
    if (!searchQuery.trim()) {
      return list;
    }
  ...
  const subtotal = React.useMemo(
    () =>
      cartItems.reduce((total, line) => {
        const price =
          line.product.type === "GOODS"
            ? (line.product.goods?.sellingPrice ?? 0)
            : (line.product.service?.sellingPrice ?? 0);
  ...
  const handleAddProduct = (product: POSProduct) => {
    ...
    setCart((prev) => {
      ...
      // Check stock only for GOODS type products
      if (product.type === "GOODS" && product.goods) {
        const currentStock = product.goods.currentStock ?? 0;
  ...
  const handleSubmitOrder = async () => {
    ...
    if (
      serviceLine &&
      (!serviceLine.bookingSlotId || !serviceLine.bookingStart || !serviceLine.bookingEnd)
    ) {
  ...
  return (
    <div className="text-slate-900 transition-colors dark:text-slate-100">
        ...
        <header className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm text-slate-600 dark:text-slate-400">
              {outletData.name} · {outletData.address || "Alamat belum diisi"}
            </p>
          </div>
          ...
          <Card className="border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
            <CardContent className="flex flex-col gap-1 py-4">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {cashSummary.transactionsCount} transaksi cash •{" "}
                {new Date(cashSummary.startTime).toLocaleDateString("id-ID", {
                  weekday: "long",
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                  ...
                  {cartItems.length === 0 ? (
                    ...
                    cartItems.map((line) => {
                      ...
                      const scheduleDate = hasSchedule
                        ? slotDateFormatter.format(new Date(line.bookingStart!))
                        : null;
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-slate-600 dark:text-slate-400">
                                Rp {currencyFormatter.format(linePrice)}
                              </p>
                            </div>
                            <div className="text-right">
                              ...
                              {line.quantity > 1 && (
                                <p className="text-xs text-slate-500 dark:text-slate-400">
                                  {line.quantity} × Rp {currencyFormatter.format(linePrice)}
                                </p>
                              )}
                            </div>
                          </div>
                ...
                <div className="mt-auto flex flex-col gap-2">
                  ...
                  {!canSubmit && cartItems.length > 0 && (
                    <p className="text-xs text-red-500 dark:text-red-400">
                      Pastikan data pelanggan, jadwal layanan, dan pembayaran sudah lengkap sebelum
                      membuat pesanan.
                    </p>
                  )}
                </div>
      ...
      <Dialog
        open={Boolean(onlinePaymentResult)}
        onOpenChange={(value) => !value && resetPaymentState()}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Instruksi Pembayaran Online</DialogTitle>
            <DialogDescription>
              Bagikan instruksi berikut ke pelanggan dan pantau status pembayaran secara real-time
              dari dashboard.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-6">
              {(isPaymentSettled || isPaymentRejected) && (
                <section
                  ...
                  <p className="font-semibold">
                    {isPaymentSettled
                      ? "Pembayaran berhasil! Pesanan siap diproses."
                      : "Pembayaran gagal. Silakan bantu pelanggan memilih opsi lain."}
                  </p>
                  <p className="text-sm">
                    {isPaymentSettled
              ...
              <section className="grid gap-4 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/60 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Dibuat {formatDateTime(onlinePaymentResult.order.createdAt)}
                  </p>
                </div>
                <div className="space-y-2">
                  <p className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Total bayar termasuk fee: Rp{" "}
                    {currencyFormatter.format(onlinePaymentResult.order.totalAmount)}
                  </p>
                </div>
              </section>
              ...
              {onlinePaymentResult.transaction.midtrans ? (
                <section className="space-y-4">
                    ...
                    <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/60">
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex-1">
                          ...
                          <p className="mt-1 font-mono text-lg font-semibold tracking-wide text-slate-900 dark:text-slate-100">
                            {onlinePaymentResult.transaction.midtrans.paymentCode}
                          </p>
                        </div>
                        <Button
                      ...
                      <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                        Berikan kode ini kepada pelanggan untuk disalin ke aplikasi pembayaran.
                      </p>
                    </div>
                  )}
                    ...
                    <div className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/60">
                      ...
                      {onlinePaymentResult.transaction.midtrans.vaNumbers.map((va) => (
                        <div
                          ...
                          className="flex items-center justify-between gap-4 rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-950">
                          <div className="flex-1">
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                              {va.bank?.toUpperCase()}
                            </p>
                  ...
                  {onlinePaymentResult.transaction.midtrans.instructions?.length ? (
                    <div className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/60">
                      ...
                      {onlinePaymentResult.transaction.midtrans.instructions.map(
                        (instruction, index) => (
                          <div
                            key={`${instruction.title}-${index}`}
                            className="space-y-2 rounded-md border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-950/60">
                            <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                              {instruction.title}
                            </p>
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-300">
                  Detail pembayaran online belum tersedia. Silakan cek dashboard transaksi atau
                  hubungi admin.
                </div>
              )}
              ...
              <div className="rounded-lg border border-orange-300 bg-orange-50 p-4 text-sm text-orange-800 dark:border-orange-500/40 dark:bg-orange-500/10 dark:text-orange-200">
                <div className="flex items-start gap-2">
                  ...
                  <p>
                    Transaksi akan kadaluarsa{" "}
                    {formatDateTime(onlinePaymentResult.transaction.midtrans?.expiredAt)}. Pastikan
                    pelanggan menyelesaikan pembayaran sebelum waktu tersebut.
                  </p>
                </div>
              </div>
            </div>
