...
export default function OrderDetailModal({ order, isOpen, onClose }: OrderDetailModalProps) {
  ...
  const scheduledStart =
    order.queueMeta?.scheduledStart ?? order.bookingSlot?.startTime ?? order.bookingDate ?? null;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md sm:rounded-md p-0 max-w-full rounded-none">
        <DialogHeader className="p-6 pb-4">
          <DialogTitle className="flex items-center gap-2">Detail Pesanan</DialogTitle>
          ...
          <div className="space-y-2 p-4 rounded-lg bg-muted/50">
            ...
            {order.orderStatus === "CANCELLED" && (
              <div className="pt-2 mt-2 border-t">
                <span className="text-sm text-destructive font-medium block mb-1">
                  Alasan Pembatalan:
                </span>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm capitalize">
                {order.transaction?.paymentMethod.replace(/_/g, " ") || "Uknown"}
              </span>
            </div>
          </div>
        </div>
