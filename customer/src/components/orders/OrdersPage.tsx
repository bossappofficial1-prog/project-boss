...
const getOrderScheduleWindow = (order: OrderDetail) => {
  ...
  const endCandidate =
    order.bookingSlot?.endTime ?? order.queueMeta?.scheduledEnd ?? null;
  if (endCandidate) {
  ...
  const duration =
    order.items.find((i) => i.product.type === "SERVICE")?.product.service
...
const generateIcsContent = (order: OrderDetail) => {
  ...
  const primaryItem =
    order.items.find((i) => i.product.type === "SERVICE") ?? order.items[0];
...
const isOrderStatusValue = (
  v: string | undefined | null,
): v is OrderStatusType =>
      ...
      if (err instanceof AxiosError) {
        ...
        if (typeof msg === "string") {
          const map: Record<string, () => string> = {
            ...
            "Pesanan belum siap untuk dikonfirmasi": () =>
              t("messages.errors.confirmNotReady"),
          };
