...
import { mapOutletsWithOpenStatus } from "../utils/outlet.utils";
...
export class AdminRepository {
  static async getDashboardMetrics() {
    ...
    const businessesChange =
      previousMonthBusinesses > 0
        ? ((totalBusinesses - previousMonthBusinesses) /
    ...
    const [orders] = await Promise.all([
      db.order.findMany({
        ...
        where: {
          ...
          ...(type &&
            type === "order_completed" && { orderStatus: "COMPLETED" }),
        },
        include: {
          outlet: {
            select: {
  ...
  static async getAllOutlets(options: {
    ...
    const withStatus = mapOutletsWithOpenStatus(allOutlets);

    const filtered =
      status === "OPEN"
        ? withStatus.filter((o) => o.isOpen)
        : status === "CLOSED"
          ? withStatus.filter((o) => !o.isOpen)
          : withStatus;

    const total = filtered.length;
    const outlets = filtered.slice(skip, skip + limit);
  ...
  static async getRevenueAnalyticsReport(options: {
    ...
    const formattedRevenueData = Object.entries(monthlyRevenue).map(
      ([period, data]) => ({
        period,
    ...
    const businessesWithRevenue = topBusinesses
      .map((business) => {
        const totalRevenue = business.outlets.reduce((businessSum, outlet) => {
          return (
            businessSum +
            outlet.orders.reduce(
              (outletSum: number, order: any) => outletSum + order.totalAmount,
              0,
            )
          );
        }, 0);
        const totalOrders = business.outlets.reduce(
          (count, outlet) => count + outlet.orders.length,
          0,
        );
  ...
  static async getRevenueInsights() {
    ...
    const averageOrderValue = Number(
      monthOrderAggregate._avg?.totalAmount || 0,
    );
    const previousRevenue = Number(
      prevMonthRevenueAggregate._sum.totalAmount || 0,
    );
    const revenueGrowth =
      previousRevenue > 0
    ...
    const paymentTotal = paymentMethodsRaw.reduce(
    ...
    const planTotal = planDistributionRaw.reduce(
    ...
    const lifetimeMidtransFee = Number(
      lifetimeOrderAggregate._sum.midtransFee || 0,
    );
  ...
  static async getSubscriptionIncomeOverview(options: { months: number }) {
    ...
    const averageContractValue =
      invoicesThisMonth > 0 ? mrr / invoicesThisMonth : 0;

    const revenueTrend = invoiceTrendRaw.map((item) => ({
      date:
        item.period instanceof Date
          ? item.period.toISOString()
          : String(item.period),
      revenue: Number(item.revenue || 0),
      invoices: Number(item.invoices || 0n),
    }));

    const totalActiveBusinesses = planCounts.reduce(
      (sum, plan) => sum + plan._count._all,
      0,
    );
      ...
      return {
        ...
        percentage:
          totalActiveBusinesses > 0
            ? (businesses / totalActiveBusinesses) * 100
            : 0,
    ...
    const upcomingRenewals = upcomingRenewalsRaw.map((renewal) => ({
      ...
      daysRemaining: Math.max(
        0,
        Math.ceil(
          (renewal.endDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
        ),
      ),
    }));
    ...
    const recentInvoices = recentInvoicesRaw.map((invoice) => ({
      ...
      planName:
        ...
        (invoice.business?.subscriptionPlan ?? "plan"),
    }));
    ...
    if (reportType === "summary") {
      ...
      return {
        ...
        netRevenue:
          ...
          (appFees._sum.appFee || 0) -
          (midtransFees._sum.midtransFee || 0),
      };
  ...
  static async getSystemHealth() {
    ...
    const cpuUsage =
      totalCpuTime > 0 ? ((totalCpuTime - idle) / totalCpuTime) * 100 : 0;

    const systemMetrics = {
      status:
        dbHealthCheck.status === "connected"
          ? "healthy"
          : ("unhealthy" as "healthy" | "warning" | "unhealthy" | "unknown"),
      uptime,
      memory: {
  ...
  static async getBusinessPerformanceReport(options: {
    ...
    const businessReports = businesses.map((business: any) => {
      const allOrders = business.outlets.flatMap(
        (outlet: any) => outlet.orders,
      );
      ...
      const totalCommission = allOrders.reduce(
        (sum: number, order: any) => sum + (order.appFee || 0),
        0,
      );
