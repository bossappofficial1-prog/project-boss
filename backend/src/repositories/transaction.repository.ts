...
export async function findExpensesByFilter(filter: { outletId?: string; userOutletIds: string[]; startDate?: string; endDate?: string; query?: string; }) {
...
export async function computeTotalsByFilter(filter: { outletId?: string; userOutletIds: string[]; status?: string; startDate?: string; endDate?: string; query?: string; }, revenueStatuses?: string[]) {
    const { outletId, userOutletIds, status, startDate, endDate, query } = filter;
    ...
    if (normalizedStatuses?.length) {
        orderWhere.paymentStatus = normalizedStatuses.length === 1
            ? normalizedStatuses[0]
            : { in: normalizedStatuses };
        orderWhere.transaction = {
            status: normalizedStatuses.length === 1
                ? normalizedStatuses[0]
                : { in: normalizedStatuses }
        };
    } else {
...
export async function computeCountsByFilter(filter: { outletId?: string; userOutletIds: string[]; status?: string; startDate?: string; endDate?: string; query?: string; }) {
    const { outletId, userOutletIds, status, startDate, endDate, query } = filter;
