...
function mapOrderToEntry(order: any, position: number): QueueEntry {
  ...
  const allItems: QueueEntryItem[] = (order.items ?? []).map((item: any) => ({
    ...
    duration:
      item.product?.type === "SERVICE" ? (item.product?.service?.durationMinutes ?? null) : null,
  }));
...
export class QueueV2Service {
  static async getBoard(
    ...
    const [activeOrders, completedOrders, cancelledCount] = await Promise.all([
      QueueV2Repository.getActiveQueueByOutlet(outletId, q, date),
      QueueV2Repository.getCompletedTodayByOutlet(outletId, q, date),
      QueueV2Repository.getCancelledTodayCount(outletId, date),
    ]);
