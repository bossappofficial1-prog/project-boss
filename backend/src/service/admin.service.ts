...
export class AdminService extends BaseService {
  ...
  static async getAllBusinesses(options: {
    ...
    const businessesWithOrders = businesses.map((business) => ({
      ...
      _count: {
        ...business._count,
        orders:
          business.outlets?.reduce(
            (sum, outlet) => sum + (outlet._count?.orders || 0),
            0,
          ) || 0,
      },
    }));
  ...
  static async getBusinessDetails(businessId: string) {
    ...
    return {
      ...
      statistics: {
        ...statistics,
        totalOrders: business.outlets.reduce(
          (sum, outlet) => sum + (outlet._count?.orders || 0),
          0,
        ),
        totalOutlets: business._count.outlets,
        totalProducts: business.outlets.reduce(
          (sum, outlet) => sum + (outlet._count?.products || 0),
          0,
        ),
      },
    };
  ...
  static async getRevenueAnalytics(options: {
    ...
    if (!allowedPeriods.includes(period)) {
      throw new AppError(
        `Invalid period. Must be one of: ${allowedPeriods.join(", ")}`,
    ...
    if (startDate) {
      ...
      if (isNaN(start.getTime())) {
        throw new AppError(
          "Invalid startDate format. Use ISO date string (YYYY-MM-DDTHH:mm:ss.sssZ)",
          HttpStatus.BAD_REQUEST,
        );
      }
    }
    ...
    if (endDate) {
      ...
      if (isNaN(end.getTime())) {
        throw new AppError(
          "Invalid endDate format. Use ISO date string (YYYY-MM-DDTHH:mm:ss.sssZ)",
          HttpStatus.BAD_REQUEST,
        );
      }
    }
  ...
  static async getAllUsers(options: {
    ...
    const { page = 1, limit = 10, role, search, status } = options;
  ...
  static async getFinancialReports(options: {
    ...
    if (!allowedReportTypes.includes(reportType)) {
      throw new AppError(
        `Invalid report type. Must be one of: ${allowedReportTypes.join(", ")}`,
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  static async getSupportTickets(options: {
    ...
    if (priority && !allowedPriorities.includes(priority)) {
      throw new AppError(
        `Invalid priority. Must be one of: ${allowedPriorities.join(", ")}`,
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  static async updatePlatformSettings(settings: any, adminId: string) {
    ...
    if (settings.platform) {
      ...
      if (maintenance !== undefined && typeof maintenance !== "boolean") {
        throw new AppError(
          "Platform maintenance must be a boolean",
          HttpStatus.BAD_REQUEST,
        );
      }
    }
    ...
    if (settings.fees) {
      const { appFeePercentage, midtransFee, minimumWithdrawal } =
        settings.fees;
      if (
        appFeePercentage !== undefined &&
        (typeof appFeePercentage !== "number" ||
          appFeePercentage < 0 ||
          appFeePercentage > 100)
      ) {
        throw new AppError(
          "App fee percentage must be a number between 0 and 100",
          HttpStatus.BAD_REQUEST,
        );
      }
      if (
        midtransFee !== undefined &&
        (typeof midtransFee !== "number" || midtransFee < 0)
      ) {
        throw new AppError(
          "Midtrans fee must be a positive number",
          HttpStatus.BAD_REQUEST,
        );
      }
      if (
        ...
        throw new AppError(
          "Minimum withdrawal must be a positive number",
          HttpStatus.BAD_REQUEST,
        );
      }
    }
    ...
    if (settings.notifications) {
      ...
      if (
        ...
        throw new AppError(
          "WhatsApp enabled must be a boolean",
          HttpStatus.BAD_REQUEST,
        );
      }
      if (emailEnabled !== undefined && typeof emailEnabled !== "boolean") {
        throw new AppError(
          "Email enabled must be a boolean",
          HttpStatus.BAD_REQUEST,
        );
      }
    }
    ...
    if (settings.limits) {
      const {
        ...
        maxOutletsPerBusiness,
        maxProductsPerOutlet,
      } = settings.limits;
      if (
        maxBusinessesPerOwner !== undefined &&
        (typeof maxBusinessesPerOwner !== "number" || maxBusinessesPerOwner < 1)
      ) {
        throw new AppError(
          "Max businesses per owner must be a positive number",
          HttpStatus.BAD_REQUEST,
        );
      }
      if (
        maxOutletsPerBusiness !== undefined &&
        (typeof maxOutletsPerBusiness !== "number" || maxOutletsPerBusiness < 1)
      ) {
        throw new AppError(
          "Max outlets per business must be a positive number",
          HttpStatus.BAD_REQUEST,
        );
      }
      if (
        maxProductsPerOutlet !== undefined &&
        (typeof maxProductsPerOutlet !== "number" || maxProductsPerOutlet < 1)
      ) {
        throw new AppError(
          "Max products per outlet must be a positive number",
          HttpStatus.BAD_REQUEST,
        );
      }
    }
  ...
  static async getAllOrders(options: {
    ...
    const { page = 1, limit = 10, search, status, paymentStatus } = options;

    const { orders, total } = await AdminRepository.getAllOrders({
      page,
      limit,
      search,
      status,
