...
export async function getProductByIdService(id: string) {
  ...
  // Transform response based on product type
  const { outlet, ...productData } = product;

  // Get images from productImages relation
  const images =
...
export async function updateProductService(id: string, data: UpdateProductInput) {
  ...
  // If service duration changed, regenerate booking slots
  if (data.service?.durationMinutes && product.service) {
    const prevDuration = existingProduct.service?.durationMinutes;
  ...
  try {
    if (isZip && file.path) {
      ...
      excelPath =
        ...
        null;
      imagesDir = fs.existsSync(path.join(workDir!, "images"))
    ...
    data.forEach((row: any, index: number) => {
      ...
      const rawFeeBearer = normalizeEnum<FeeBearer>(
        row["Penanggung Biaya"],
        Object.values(FeeBearer),
      );
...
export async function exportProductsToExcelService(
  ...
  const data = products.map((product, index) => [
    ...
    "N/A",
    (product as any).transactionFeeBearer === "CUSTOMER"
      ? "Pelanggan"
      : (product as any).transactionFeeBearer === "OWNER"
        ? "Pemilik"
        : "Default Bisnis",
