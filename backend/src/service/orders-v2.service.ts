...
export class OrdersV2Service {
  static async getBoard(
    ...
    const [activeOrders, completedOrders, todayStats] = await Promise.all([
      OrdersV2Repository.getActiveOrdersByOutlet(outletId, q, dateStr),
      OrdersV2Repository.getCompletedTodayByOutlet(outletId, q, dateStr),
      OrdersV2Repository.getTodayStats(outletId, dateStr),
    ]);
  ...
  static async getBadgeQueueAndOrderCount(outletId: string) {
    const [orderBadgeCount, serviceBadgeCount] =
      await OrdersV2Repository.getBadgeQueueAndOrderCount(outletId);
