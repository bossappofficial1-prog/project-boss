...
export async function createOrderRecord(data: CreateOrderInput): Promise<OrderCreationResult> {
  ...
  if (staffId) {
    ...
    if (staff.outletId !== outletId) throw new AppError("Staff does not belong to this outlet.", HttpStatus.FORBIDDEN);
    if (staff.status !== StaffStatus.ACTIVE) throw new AppError("Staff is currently inactive.", HttpStatus.BAD_REQUEST);
  }
  ...
  if (slot && slotStart && slotEnd) {
    ...
    if (staffAvailability.length <= 0) {
      throw new AppError("No staff assigned to this service window.", HttpStatus.CONFLICT);
    }

    if (availableStaffCount <= 0) {
      throw new AppError("No staff available for this slot.", HttpStatus.CONFLICT);
    }
    ...
    if (handledByStaffId) {
      ...
      if (!isStaffAvailable) {
        throw new AppError("Selected staff is not available for this time.", HttpStatus.CONFLICT);
      }
    }
  }
  ...
  return db.$transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      if (!product || product.outletId !== outletId || product.status !== "ACTIVE") {
        throw new AppError(`Product ${item.productId} is unavailable or inactive.`, HttpStatus.BAD_REQUEST);
      }

      let price: number;
      if (product.type === "GOODS") {
        if (!product.goods) throw new AppError(`Data missing for goods: ${product.name}`, HttpStatus.INTERNAL_SERVER_ERROR);
        if (product.goods.currentStock < item.quantity) {
          throw new AppError(`Insufficient stock for ${product.name}. Available: ${product.goods.currentStock}`, HttpStatus.BAD_REQUEST);
        }
        price = product.goods.sellingPrice;
      } else {
        if (!product.service) throw new AppError(`Data missing for service: ${product.name}`, HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    if (subTotal > 50000000) throw new AppError("Maximum order is Rp 50.000.000", HttpStatus.BAD_REQUEST);

    // Fee Calculations
    const isDigitalPayment = data.paymentMethod === "online" || (data.paymentMethod === "qris" && !!data.onlinePaymentChannel);
    ...
    if (serviceItems.length > 0 && !bookingSlotId) {
      if (!data.bookingDate) throw new AppError("Service bookings require a date/time.", HttpStatus.BAD_REQUEST);
      ...
      for (const svc of serviceItems) {
        ...
        const existingOverlaps = await tx.order.count({
          where: {
            outletId,
            orderStatus: { in: [OrderStatus.AWAITING_PAYMENT, OrderStatus.PROCESSING, OrderStatus.CONFIRMED] },
            items: { some: { productId: svc.id } },
            OR: [
              { bookingDate: { lt: requestedEnd, gte: requestedStart } },
              { bookingSlot: { startTime: { lt: requestedEnd }, endTime: { gt: requestedStart } } }
            ]
          }
        });

        if (existingOverlaps >= maxParallel) {
          throw new AppError(`No capacity left for ${svc.name} at the requested time.`, HttpStatus.CONFLICT);
        }
      }
    }

    // 6. Customer Management
    let customer = await tx.guestCustomer.findFirst({ where: { phone: data.guestCustomer.phone } });
    if (!customer) {
    ...
    await tx.orderItem.createMany({
      data: items.map((item) => {
        ...
        const price = prod?.type === "GOODS" ? prod.goods.sellingPrice : prod.service.sellingPrice;
        return {
          orderId: order.id,
