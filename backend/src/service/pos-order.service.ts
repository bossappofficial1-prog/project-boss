...
import {
  buildMidtransCorePayload,
  normalizeMidtransCoreResponse,
} from "../utils/midtrans-core.utils";
...
function assertOrder(
  order: Awaited<ReturnType<typeof OrderRepository.findById>> | null,
): OrderWithRelations {
  if (!order) {
...
async function handleManualQrisOrder(
...
async function handleOnlineOrder(
  ...
  const { order, midtransFee, appFee, feeBearer, totalAmount } =
