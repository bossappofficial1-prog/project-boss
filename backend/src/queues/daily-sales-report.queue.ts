...
export class DailySalesReportQueue extends BaseQueue<DailySalesReportPayload> {
  ...
  protected async handle(job: Job<DailySalesReportPayload>): Promise<void> {
    logger.info(
      "📊 Mulai pengolahan laporan penjualan harian untuk seluruh owner...",
      {
    ...
    try {
      ...
      const generatedAtStr = format(new Date(), "dd MMMM yyyy HH:mm:ss", {
        locale: localeId,
      });
      ...
      const activeBusinesses =
        await BusinessRepository.findAllActiveWithOwnerAndOutlets();

      logger.info(
        `🔍 Menemukan ${activeBusinesses.length} bisnis aktif untuk diproses.`,
        {
      ...
      for (const business of activeBusinesses) {
        if (!business.owner?.email) {
          logger.warn(
            `⚠️ Bisnis ${business.name} tidak memiliki email owner. Melewati...`,
          );
          continue;
        }

        logger.info(
          `📈 Memproses laporan untuk bisnis: ${business.name} (${business.owner.email})`,
        );
        ...
        for (const outlet of business.outlets) {
          ...
          const outletProfitMargin =
            revenue > 0 ? roundTo((netProfit / revenue) * 100, 2) : 0;
          ...
          outletsData.push({
            ...
            transactionCount,
            profitMargin: outletProfitMargin,
          });
        }

        if (totalTransactions === 0) {
          logger.info(
            `📉 Tidak ada transaksi untuk bisnis ${business.name} pada tanggal ${dateStr}. Melewati pengiriman email.`,
          );
          continue;
        }

        const netProfit = totalRevenue - totalExpense;
        const outletCount = business.outlets.length;
        const avgRevenuePerTrx =
          totalTransactions > 0
            ? Math.round(totalRevenue / totalTransactions)
            : 0;
        const expenseRatio =
          totalRevenue > 0
            ? roundTo((totalExpense / totalRevenue) * 100, 2)
            : 0;
        const profitMargin =
          totalRevenue > 0 ? roundTo((netProfit / totalRevenue) * 100, 2) : 0;
        ...
        logger.info(
          `📧 Email laporan penjualan harian (dengan PDF) berhasil dikirim ke ${business.owner.email} (${business.name})`,
        );
      }

      logger.info(
        "✅ Selesai memproses laporan penjualan harian untuk seluruh owner.",
      );
    } catch (error: any) {
